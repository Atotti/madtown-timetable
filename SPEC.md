...
```typescript
{
  ...
  streams: Array<{
    ...
    startTime: string;             // 配信開始日時（actualStartTimeから取得、ISO 8601形式）
    endTime?: string;              // 配信終了日時（actualEndTimeから取得、ISO 8601形式、配信中はnull）
...
**注意**: `startTime`と`endTime`は`liveStreamingDetails.actualStartTime`と`actualEndTime`から取得します。通常のアップロード動画（`liveStreamingDetails`が存在しない）は除外されます。
...
```mermaid
graph LR
    A[channels.json] -->|各チャンネルごと| B[YouTube Data API]
    B -->|search.list| C[動画検索]
    C -->|タイトルフィルタ| D[対象動画抽出]
    D -->|videos.list| E[詳細情報取得]
    E --> F[streams.json生成]
```

**YouTube Data API使用**:
1. `search.list` エンドポイント
   - チャンネルIDごとに検索
   - クエリ: タイトルに"GTA"または"MADTOWN"を含む
   - 期間フィルタ: `config.json`の開始日〜終了日
   - `type=video`、`eventType=completed`（アーカイブ）と`eventType=live`（配信中）

2. `videos.list` エンドポイント（`part=liveStreamingDetails,snippet,statistics`）
...
- **API**: YouTube Data API v3（公式クライアントライブラリ使用）
- **HTTP**: axios または fetch
